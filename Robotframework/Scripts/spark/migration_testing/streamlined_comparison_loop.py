...
table_name='2_deterministic_matching'
...
table_name_list = ['temp_wca_dgr', 'temp_pt_account_clean', 'temp_wlp_client_clean', 'temp_wisi_active_policies', 'deterministic_matches']

for n, tbl_nm_elem in enumerate(table_name_list):

    table_name = tbl_nm_elem

    ####### Update this cell #######

    # Add testing specific path, you can hardcode it completely
    tgt_table_cz_path = f'{cz_base_path}/four_pillar/{table_name}/'
    ...
    dates_needed.extract_config_data(config_file_path, folder_path_config, table_name, pod_name, extract_prep_flag)
    ...
    src_df = spark.sql(f"""
                        select *
                        from wealth_migration_db.onprem_{table_name} 
    ...
    result_summary_indiv_df = spark.sql(f"""
                                                    select * from global_temp.temp_{table_name}_regression_result_summary
                                                    union all
                                                    select * from global_temp.temp_{table_name}_unittest_result_summary
                                                """)
    if n == 0:
        result_summary_final_df = result_summary_indiv_df
    else:
        result_summary_final_df = result_summary_final_df.union(result_summary_indiv_df)
...
result_summary_final_df.select('type', 'table_name', 'column_name', 'overall_Status',
...
overall_status, overall_file_path = external_functions.get_final_overall_status(table_name, dbfs_folder_base_path=dbfs_folder_base_path, overall_df=result_summary_final_df)
