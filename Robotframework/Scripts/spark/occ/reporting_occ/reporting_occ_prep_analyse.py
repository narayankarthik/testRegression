...
# actual
# sec_offset_df = spark.sql(f"select * from delta.`{cz_base_path}/occ/security_offset_occ`")
# to_test
sec_offset_df = spark.read.csv(f'dbfs:/FileStore/tables/DaaSWealth_QA/Mainkar/Config/data/security_offset_occ_{as_of_date}.csv', header='True')
...
so_3_1_df.orderBy('bor_client_id', 'option_type', 'underlying_symbol', 'row_id').display()
...
so_4_df.orderBy('bor_client_id', 'option_type', 'underlying_symbol', 'row_id').display()
...
order_spec = Window.partitionBy('bor_client_id', 'option_type', 'symbol')\
                    .orderBy('bor_client_id', 'option_type', 'symbol', 'underlying', 'expiry_maturity_date')    #10
...
# Tgt part
# Below lines are to test all the individual steps, if needed
# tgt_df = spark.sql(f"""select * from delta.`abfss://wcz0006@edaaawcze1stgcz.dfs.core.windows.net/curated/wcz0006/occ/security_offset_occ_step3_2_long`""")
# tgt_df = spark.sql(f"""select * from delta.`abfss://wcz0006@edaaawcze1stgcz.dfs.core.windows.net/curated/wcz0006/occ/security_offset_occ_step3_4_short`""")
# tgt_df = spark.sql(f"""select * from delta.`abfss://wcz0006@edaaawcze1stgcz.dfs.core.windows.net/curated/wcz0006/occ/security_offset_occ_step4`""").drop('long', 'short')
# tgt_df = spark.sql(f"""select * from delta.`abfss://wcz0006@edaaawcze1stgcz.dfs.core.windows.net/curated/wcz0006/occ/security_offset_occ_step5`""")
# tgt_df = spark.sql(f"""select * from delta.`abfss://wcz0006@edaaawcze1stgcz.dfs.core.windows.net/curated/wcz0006/occ/security_offset_occ_step6_1`""")
# tgt_df = spark.sql(f"""select * from delta.`abfss://wcz0006@edaaawcze1stgcz.dfs.core.windows.net/curated/wcz0006/occ/security_offset_occ_step6_2`""")
# tgt_df = spark.sql(f"""select * from delta.`abfss://wcz0006@edaaawcze1stgcz.dfs.core.windows.net/curated/wcz0006/occ/security_offset_occ_step7_2`""")\
#                 .withColumnRenamed('name_col', 'short_or_long').withColumnRenamed('value_col', 'quantity')
# tgt_df = spark.sql(f"""select * from delta.`abfss://wcz0006@edaaawcze1stgcz.dfs.core.windows.net/curated/wcz0006/occ/security_offset_occ_step7_4`""")
# tgt_df = spark.sql(f"""select * from delta.`abfss://wcz0006@edaaawcze1stgcz.dfs.core.windows.net/curated/wcz0006/occ/security_offset_occ_step8_3`""").drop('shortbalance_temp', 'max_shortbalance')
# tgt_df = spark.sql(f"""select * from delta.`abfss://wcz0006@edaaawcze1stgcz.dfs.core.windows.net/curated/wcz0006/occ/security_offset_occ_step9_3`""")
# order_spec = Window.partitionBy('bor_client_id', 'option_type', 'underlying_symbol')\
#                     .orderBy('bor_client_id', 'option_type', 'underlying_symbol', 'underlying', 'long_option_maturitydate', 'short_option_maturitydate')    #4, 5, 6.1, 6.2, 7.4
                    # .orderBy('bor_client_id', 'option_type', 'underlying_symbol', 'underlying', 'long_option_maturitydate', 'short_option_maturitydate', 'quantity')    #7.2
                    # .orderBy('bor_client_id', 'option_type', 'underlying_symbol', 'underlying', 'option_maturitydate')  #3.2, 3.4
# Actual final target table
tgt_df = spark.sql(f"""select * from delta.`{tgt_table_cz_path}`""")
order_spec = Window.partitionBy('bor_client_id', 'option_type', 'symbol')\
                    .orderBy('bor_client_id', 'option_type', 'symbol', 'underlying', 'expiry_maturity_date')    #10
...
#                 dbfs_folder_base_path='/dbfs/FileStore/tables/DaaSWealth_QA/Mainkar/', passed_as_of_date='20230425', extract_prep_flag='prep')
