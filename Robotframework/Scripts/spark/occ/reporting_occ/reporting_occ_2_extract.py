...
def execute(table_name='reporting_occ', script_type='super_script', config_file_path='', env='cz_staging', pod_name='dc',
            dbfs_folder_base_path='/dbfs/FileStore/tables/DaaSWealth_QA/Mainkar/', passed_as_of_date='20230927', extract_prep_flag='extract'):
    ...
    rep_occ_df = spark.sql(f"select * from delta.`{tgt_table_cz_path}`")\
                        .filter(f'process_date="{process_date}"')
    rep_occ_df.createOrReplaceTempView('temp_qa_{0}'.format(rep_occ_table_name))
    print(f'View created:temp_qa_{rep_occ_table_name}')
    print("rep_occ_df count: {0}".format(rep_occ_df.count()))
    ...
    #                             select 'O' as type,
    #                                 qa_symbol as symbol,
    #                                 qa_occ_strkprice_report as strike,
    #                                 qa_p_c as p_c,
    #                                 qa_series_contract_date as series_contract_date,
    #                                 '' as cusip,
    #                                 qa_occ_qty_reported as net_quantity,
    #                                 '' as settlement_date,
    #                                 '' as net_cash,
    #                                 qa_occ_qty_reported as occ_qty_reported
    #                             from temp_qa_{rep_occ_table_name}
    #                         """)
    src_final_df = rep_occ_df.select('symbol', 'occ_strkprice_report', 'p_c', 'series_contract_date', 'occ_qty_reported')\
                                .withColumn('type', lit('O'))\
                                .withColumn('cusip', lit(''))\
                                .withColumn('settlement_date', lit(''))\
                                .withColumn('net_cash', lit(''))\
                                .withColumn('net_quantity', col('occ_qty_reported'))\
                                .withColumnRenamed('occ_strkprice_report', 'strike')
    order_spec = Window.orderBy('type', 'symbol', 'strike', 'p_c', 'series_contract_date', 'cusip',
                                'net_quantity', 'settlement_date', 'net_cash', 'occ_qty_reported')
    src_final_df = src_final_df.withColumn('rnm', row_number().over(order_spec))
    for col_name in src_final_df.columns:
        src_final_df = src_final_df.withColumnRenamed(col_name, f'qa_{col_name}')
    ...
    try:
        ...
        spark.sql(f"drop table if exists wdar_qa.qa_{table_name}{table_suffix}_final")
        src_final_df.write.format('delta').saveAsTable(f'wdar_qa.qa_{table_name}{table_suffix}_final', path=f'{src_table_cz_path}')
    print(f"Have overwriten the new records to wdar_qa.qa_{table_name}{table_suffix}_final.")
    # creating a view
    ...
    # tgt_df = tgt_df.withColumn('strike', col('strike').cast(DecimalType(30,6)))
