...
def execute(table_name='reporting_occ', script_type='super_script', config_file_path='', env='cz_staging', pod_name='dc',
    ...
    # reverse the comment statements later
    # src_final_df = rep_occ_df.select('qa_process_date', 'qa_bor_client_id', 'qa_symbol', 'qa_option_Type', 'qa_expiry_maturity_date',
    #                                 'qa_occ_qty_reported', 'qa_txntyp', 'qa_actn', 'qa_id_clr', 'qa_id_acct', 'qa_typ', 'qa_occ_strkprice_report')\
    #                             .withColumnRenamed('qa_occ_strkprice_report', 'qa_strkpx')
    src_final_df = rep_occ_df
    src_final_df = src_final_df.select('process_date', 'bor_client_id', 'symbol', 'option_Type',
                                        'expiry_maturity_date', 'occ_qty_reported', 'txntyp',
                                        'actn', 'id_clr', 'id_acct', 'typ', 'occ_strkprice_report')\
                                .withColumnRenamed('occ_strkprice_report', 'strkpx')
    for col_name in src_final_df.columns:
        src_final_df = src_final_df.withColumnRenamed(col_name, f'qa_{col_name}')
    order_spec = Window.partitionBy('qa_bor_client_id', 'qa_symbol', 'qa_option_Type', 'qa_expiry_maturity_date')\
                        .orderBy('qa_strkpx','qa_occ_qty_reported')
    ...
    order_spec = Window.partitionBy('bor_client_id', 'symbol', 'option_Type', 'expiry_maturity_date')\
                        .orderBy('strkpx', 'occ_qty_reported')
